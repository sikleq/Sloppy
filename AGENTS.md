...
- **Лого с eye-fire.** Шапочный логотип — это `<span class="nav-brand-logo">` (контейнер, не `<img>`) с fallback-`<img>` внутри + canvas-слои **EyeFire** (анимированное синее свечение «глаз» шлема + искры). Класс `EyeFire` в конце `scripts.js`, инициализация по `[data-logo-src]`/`[data-glow-src]`. Свечение берётся из **запечённой плашки** `icons/header-helmet-glow.png` (генерится PIL-скриптом из `header-helmet.png`; не `getImageData` на лету — иначе ломается на `file://`). Глаза — image-px координаты `(305,545)`/`(470,545)`. PNG лого — прозрачный фон (НЕ чёрный; старый `mix-blend:screen` убран).
