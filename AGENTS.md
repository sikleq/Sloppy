...
- `docs/architecture.md`, `docs/workflow.md`, `docs/data-format.md` — конвейер **патч-страниц** (пакет `patch/` + `content/` + `builders/patch.py`).
...
Сайт = две подсистемы: (1) аннотированные патчноуты и (2) сортируемые таблицы под разделом Materials. Общие `styles.css` / `src/scripts.js`.

## ВАЖНО: source of truth

Патч-страницы собирает пакет `patch/` (хелперы: `badges.py`, `elements.py`, `page.py`, …); контент каждого патча — в `content/p<version>.py` (`def build()`); оркестратор — `builders/patch.py`. CSS/JS читаются с диска при сборке (`patch/page.py`): `styles.css` и `src/scripts.js` — это **source files**, редактируются напрямую.
- `src/scripts.js` и `styles.css` — единственный источник правды для стилей и поведения всех страниц (включая `index.html` и `calendar.html`). Редактируй напрямую — IDE/linter работают нормально.
- Сгенерированные HTML (`patches/7.41c.html` и т.д.) — результат запуска `python builders/patch.py`, не редактируй вручную.
- Все патч-файлы лежат в `patches/` (не в корне), поэтому их CSS/JS подключаются через `../styles.css`, `../src/scripts.js`.
...
```
patch/                    ← пакет хелперов (badges, elements, page, stats, …)
content/p<version>.py     ← контент каждого патча (def build())
builders/patch.py         ← оркестратор: запускает все content.build()
builders/*.py             ← остальные страницы (creeps, mana_items, heroes_stats, …)
generate_patch_code_v2.py ← datafeed → Python-код. Запускать перед добавлением патча
data/
  <version>_datafeed.json ← датафид Valve (вход для generate_patch_code_v2.py)
...
python builders/heroes_stats.py # heroes_stats.html — таблица статов героев (после builders/patch.py)
python builders/hero_lab.py     # hero_lab.html — калькулятор сравнения героев с предметами
python generate_patch_code_v2.py 7.42  # → _generated_p_7.42_v2.py (сохранять в content/p742.py)
...
## Маппинг описаний → поля БД статов
...
Логика в `patch/elements.py` `ability_change(...)` решает между тремя визуальными режимами по identity и количеству строк:
...
Триггер-существительные: `penalty`, `downside`, `restriction`, `limitation`, `damage penalty`, `cost penalty`, `cooldown penalty`. Эвристика тегов в `generate_patch_code_v2.py` уже это спецкейсит.
...
`generate_patch_code_v2.py` привязывает `info`-поля датафида как `inline_note(...)` к родительской строке. Если у предыдущей li уже есть `extra=inline_note(...)`, новый текст дописывается через `<br>`.
...
Когда патчноут говорит `"Innate ability reworked"` — это **обязательно** `ability_change(...)`, не две плоские `t("MISC")` строки. Старое описание лифтится из патча, который ввёл текущий innate (грепать `hero_innate_<entity>_<ability>` в `patchnotes_english.txt`). Если в скаффолде остался маркер `# TODO[innate-rework]:` — разобрать его и никогда не оставлять в коммите.

## Предупреждения и подводные камни

- При добавлении нового героя: добавить в `HERO_SLUG` (`patch/images.py`). Генератор имена резолвит из `data/herolist.json` — отдельный маппинг ему не нужен.
- При добавлении нового предмета: добавить в `ITEM_SLUG` (`patch/images.py`); генератор имена берёт из `data/itemlist.json`.
