...
3. Создать Personal Access Token:
   https://github.com/settings/tokens?type=beta
       ...
       Generate token → СКОПИРОВАТЬ (показывается один раз!)
...
# ═════════════════════════════════════════════════════════════════
# НАСТРОЙКИ — отредактируй перед каждым патчем
# ═════════════════════════════════════════════════════════════════
...
def commit_batch(session, owner: str, repo: str, changes, message: str) -> str:
    """ОДИН коммит на все изменённые файлы (Git Data API: blobs → tree → commit → ref).
...
def main():
    ...
    # ── Извлекаем статы героев/юнитов/предметов ─────────────────
    ...
    # ── Заливаем в GitHub ────────────────────────────────────────
    print(f"\n☁  Синхронизирую с {GITHUB_OWNER}/{GITHUB_REPO}...")
    ...
    changes = []   # (repo_path, bytes) — всё реально изменившееся; заливается ОДНИМ коммитом
    ...
    # ── Итог: один коммит на всё ─────────────────────────────────
    print()
    if not changes:
        print("✅ Изменений не было — в репо ничего не отправлено.")
    else:
