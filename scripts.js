...
/* ============================================================================
   EyeFire — animated blue "eye-fire" on the helmet logo. Drop-in from the
   reference (Fire Effect (for Claude Code).html). Overlays a pulsing cyan glow
   on the helmet's gems/eyes + serpentine embers rising from the two eye points.
   Pauses offscreen / when the tab is hidden.
   ========================================================================== */
(function (global) {
  'use strict';
  const DEFAULTS = {
    image: null, eyes: [],
    density: 0.92, trail: 2.2, speed: 0.55, spread: 1.8, sparkScale: 0.9, glow: 1.8,
    fireColor: [150, 240, 255], pixel: 5,
    glowCyan: 30, glowBright: 120, glowBlur: 3
  };
  const instances = new WeakMap();
  function EyeFire(container, options) {
    this.host = container;
    this.cfg = Object.assign({}, DEFAULTS, options);
    if (!this.cfg.image)       throw new Error('EyeFire: options.image is required');
    if (!this.cfg.eyes.length) throw new Error('EyeFire: options.eyes is required');
    this.sparks = []; this.t = 0; this.running = false; this._raf = null;
    this._init();
  }
  EyeFire.DEFAULTS = DEFAULTS;
  EyeFire.attach = function (container, options) {
    EyeFire.detach(container);
    const inst = new EyeFire(container, options);
    instances.set(container, inst); return inst;
  };
  EyeFire.detach = function (container) {
    const inst = instances.get(container);
    if (inst) { inst.destroy(); instances.delete(container); }
  };
  EyeFire.prototype.setConfig = function (partial) { Object.assign(this.cfg, partial); return this; };
  EyeFire.prototype._init = function () {
    ...
    img.onload = () => {
      ...
      this.logoC = mk(0);
      this.glowC = mk(1, { mixBlendMode: 'screen', willChange: 'opacity, filter' });
      this.fxC   = mk(2);
      this.lctx = this.logoC.getContext('2d');
      this.gctx = this.glowC.getContext('2d');
      this.fctx = this.fxC.getContext('2d');
      this.lctx.drawImage(img, 0, 0, W, H);
      // Glow plate: prefer a PRE-BAKED PNG mask (works on file:// — no pixel
      // read); otherwise extract at runtime (needs same-origin / a server, else
      // the canvas is tainted and getImageData throws → embers still animate).
      if (this.cfg.glowMask) {
        const gm = new Image(); gm.crossOrigin = 'anonymous';
        gm.onload = () => { try { this.gctx.drawImage(gm, 0, 0, W, H); } catch (e) {} };
        gm.src = this.cfg.glowMask;
      } else {
        try { this._buildGlow(); } catch (e) { this._glowFailed = true; }
      }
      this._io = new IntersectionObserver((es) => { this._visible = es[0].isIntersecting; this._sync(); }, { threshold: 0 });
      this._io.observe(host);
      this._onVis = () => this._sync();
      document.addEventListener('visibilitychange', this._onVis);
      this._visible = true; this._sync();
    };
    img.src = this.cfg.image;
  };
  EyeFire.prototype._sync = function () {
    const want = this._visible && !document.hidden;
    if (want && !this.running) { this.running = true; this._raf = requestAnimationFrame(() => this._loop()); }
    else if (!want && this.running) { this.running = false; cancelAnimationFrame(this._raf); }
  };
  EyeFire.prototype._buildGlow = function () {
    const { W, H } = this, cfg = this.cfg;
    const t = document.createElement('canvas'); t.width = W; t.height = H;
    const tx = t.getContext('2d'); tx.drawImage(this.img, 0, 0, W, H);
    const d = tx.getImageData(0, 0, W, H).data;
    const mask = new Float32Array(W * H);
    for (let p = 0, i = 0; p < W * H; p++, i += 4) {
      const r = d[i], g = d[i + 1], b = d[i + 2], a = d[i + 3];
      const bright = (g + b) * 0.5, cyan = Math.min(g, b) - r;
      if (a > 40 && cyan > cfg.glowCyan && bright > cfg.glowBright) {
        const inten = Math.min(1, (cyan - cfg.glowCyan) / 120) * Math.min(1, (bright - cfg.glowBright) / 100 + 0.3);
        mask[p] = Math.min(1, inten * 1.4);
      }
    }
    const blurred = boxBlur(mask, W, H, Math.max(1, Math.round(cfg.glowBlur)));
    const out = this.gctx.createImageData(W, H), o = out.data;
    const [fr, fg, fb] = cfg.fireColor;
    for (let p = 0, i = 0; p < W * H; p++, i += 4) {
      o[i] = fr; o[i + 1] = fg; o[i + 2] = fb;
      o[i + 3] = Math.round(Math.min(1, blurred[p] * 1.15) * 255);
    }
    this.gctx.putImageData(out, 0, 0);
  };
  EyeFire.prototype._spawn = function () {
    const cfg = this.cfg, PIX = cfg.pixel, eyes = cfg.eyes;
    const idx = Math.random() < 0.5 ? 0 : 1 % eyes.length;
    const e = eyes[idx % eyes.length], dir = (idx % 2 === 0) ? -1 : 1;
    this.sparks.push({
      ex: e.x, ey: e.y,
      x: e.x + (Math.random() - 0.5) * PIX * 3,
      y: e.y + (Math.random() - 0.5) * PIX * 1.5, dir,
      vy: -(0.10 + Math.random() * 0.12) * PIX * cfg.speed,
      outward: 0.20 + Math.random() * 0.12,
      amp: (4 + Math.random() * 9), freq: 0.015 + Math.random() * 0.013,
      phase: Math.random() * 6.28,
      scatter: (Math.random() - 0.5) * PIX * 3,
      sscale: (Math.random() < 0.5 ? -1 : 1) * (0.5 + Math.random() * 0.9),
      life: 0, max: (150 + Math.random() * 100) * cfg.trail,
      size: (Math.random() < 0.6 ? 2 : 3) * cfg.sparkScale
    });
  };
  EyeFire.prototype._drawSparks = function () {
    const { W, H, fctx } = this, cfg = this.cfg, PIX = cfg.pixel;
    fctx.clearRect(0, 0, W, H);
    fctx.globalCompositeOperation = 'lighter';
    for (let i = this.sparks.length - 1; i >= 0; i--) {
      const s = this.sparks[i]; s.life++; s.y += s.vy;
      if (s.life > s.max) { this.sparks.splice(i, 1); continue; }
      const rise = Math.max(0, s.ey - s.y), h = rise / 300, grow = Math.pow(h, 1.7) * cfg.spread;
      s.x = s.ex + s.dir * s.outward * rise +
            s.amp * (1 + grow * 1.3) * Math.sin(rise * s.freq + s.phase) +
            s.scatter + s.sscale * grow * PIX * 10;
      const tt = s.life / s.max, a = Math.min(1, (1 - tt) * 2.0);
      let cr, cg, cb;
      if (tt < 0.3)       { cr = 245; cg = 255; cb = 255; }
      else if (tt < 0.65) { cr = 150; cg = 242; cb = 255; }
      else                { cr = 70;  cg = 185; cb = 255; }
      const r = s.size * 2.8;
      ...
      fctx.fillStyle = g;
      fctx.beginPath(); fctx.arc(s.x, s.y, r, 0, 7); fctx.fill();
    }
    fctx.globalCompositeOperation = 'source-over';
  };
  EyeFire.prototype._loop = function () {
    if (!this.running) return;
    const cfg = this.cfg; this.t++;
    if (!this._glowFailed) {
      const breathe = 0.5 + 0.5 * Math.sin(this.t * 0.028);
      const shimmer = noise(this.t * 0.2) * 0.5 + 0.5;
      this.glowC.style.opacity = Math.min(1, (0.35 + breathe * 0.5 + shimmer * 0.18) * cfg.glow).toFixed(3);
      this.glowC.style.filter = 'blur(' + (1.4 + breathe * 1.6).toFixed(2) + 'px)';
    }
    if (Math.random() < cfg.density) this._spawn();
    if (Math.random() < cfg.density * 0.3) this._spawn();
    this._drawSparks();
    this._raf = requestAnimationFrame(() => this._loop());
  };
  EyeFire.prototype.destroy = function () {
    this.running = false; cancelAnimationFrame(this._raf);
    if (this._io) this._io.disconnect();
    if (this._onVis) document.removeEventListener('visibilitychange', this._onVis);
    [this.logoC, this.glowC, this.fxC].forEach(c => c && c.remove());
  };
  function noise(x) { return Math.sin(x * 1.13) * 0.5 + Math.sin(x * 2.7 + 1.3) * 0.3 + Math.sin(x * 5.1 + 2.6) * 0.2; }
  function boxBlur(src, w, h, R) {
    const tmp = new Float32Array(w * h), out = new Float32Array(w * h), inv = 1 / (2 * R + 1);
    for (let y = 0; y < h; y++) {
      let acc = 0;
      for (let x = -R; x <= R; x++) acc += src[y * w + Math.min(w - 1, Math.max(0, x))];
      for (let x = 0; x < w; x++) { tmp[y * w + x] = acc * inv; acc += src[y * w + Math.min(w - 1, x + R + 1)] - src[y * w + Math.max(0, x - R)]; }
    }
    for (let x = 0; x < w; x++) {
      let acc = 0;
      for (let y = -R; y <= R; y++) acc += tmp[Math.min(h - 1, Math.max(0, y)) * w + x];
      for (let y = 0; y < h; y++) { out[y * w + x] = acc * inv; acc += tmp[Math.min(h - 1, y + R + 1) * w + x] - tmp[Math.max(0, y - R) * w + x]; }
    }
    return out;
  }
  global.EyeFire = EyeFire;
})(window);

/* Wire the eye-fire to the helmet logo (present in the nav on every page). */
(function () {
  const host = document.querySelector('.nav-brand-logo[data-logo-src]');
  if (!host || typeof EyeFire === 'undefined') return;
  EyeFire.attach(host, {
    image: host.dataset.logoSrc,
    glowMask: host.dataset.glowSrc || null,
    eyes: [{ x: 305, y: 545 }, { x: 470, y: 545 }],
    // The logo renders tiny (54px) — embers are drawn in image-pixel space and
    // scale down with it, so bump their size a bit to stay visible.
    sparkScale: 2.0
  });
})();
