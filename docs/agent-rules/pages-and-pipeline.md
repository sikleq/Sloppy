...
- Скриншоты: `"shots": [...]`, их может быть несколько на запись, на широком экране они стоят справа от текста. Снимаем **точечно, только само изменение**, а не весь блок или страницу. В `SHOTS` (`tools/changelog_shots.py`) задаются: страница; область (блок сущности по имени или строка по тексту); части внутри неё (CSS-селекторы, снимается их общий прямоугольник); что нажать перед съёмкой (например, открыть таблицу формулы); предел высоты. Закреплённые шапка и панели прячутся сами. Запуск: `python -m http.server 8799` из `dist/`, потом `python tools/changelog_shots.py <часть имени>`.
- Шапка: вкладка латест-патча называется **Patches**; в правом углу шапки версии патча нет, там только название таблицы на страницах Materials.
...
- **Правило:** журнал пополняется в том же коммите, что и изменение. **Новые функции и страницы** — полноценная запись: заголовок, 1–3 коротких пункта, снимки. **Мелочи** (исправления, вычитка, выравнивание) — запись с `"minor": true`: одна строка-заголовок с категорией и, по желанию, ссылкой. Мелочи дня показываются списком под записями; если их больше трёх, они сворачиваются под кнопку «Smaller changes (N)». Пишем лаконично, по-английски.
- 2 и больше скриншота в записи = карусель (стрелки, точки, ←/→ под курсором, счётчик).

## Иконки плиток главной (`tools/pixel_icons/build.py`)
- Статичные плитки: Heroes, Changelog, Stats, Hero Lab, AoE, Changes, Summons, Lane Creeps, Ko-Fi. Их иконки рисует скилл Pixel Art Studio из скрипта `tools/pixel_icons/build.py`, он и есть источник правды. Готовые PNG руками не правим, меняем скрипт и запускаем `python tools/pixel_icons/build.py --install` (PNG ляжет в `icons/ui/gothic/`).
- Стиль как у анимированных: 32×32, крупная масса на весь кадр, объём светлыми и тёмными плоскостями (свет сверху слева), контур `#311e12`, палитра `sloppy_gothic_gold` (карточка стиля в скилле: `references/learned/006-…`).
- Каждая иконка собрана из слоёв под будущую анимацию при наведении: у шлема качается плюмаж, в котле поднимаются пузыри, бусины счётов (Stats) сдвигаются, колокол (Changelog) раскачивается, кольцо стрелок Changes вращается (внутри шлем у героев и самоцвет у предметов), кольца AoE расходятся, череп поднимается, мечи скрещиваются с бликом, из чашки поднимаются сердечки.
- Dynamics у героев и у предметов различаются значком в углу (шлем / самоцвет). Скрипт накладывает его на `icon_dynamics.png` и на каждый кадр `icon_dynamics.gif`, это исходники, их не удалять. Результат: `icon_dynamics_{heroes,items}.{png,gif}`.


## Новый патч → страницы Materials (проверено 2026-09-25)
- `heroes_raw.json` (скорость атаки, обзор, снаряд, поворот, хитбокс) для раскладки 7.41f+ собирается **из локальных `data/stats/<ver>/heroes/*.txt`** (`scripts/fetch/fetch_hero_history.py → build_from_local`), шаблон `npc_dota_hero_base` — из `data/stats/<ver>/npc_dota_hero_base.txt` (достать из VPK игры), иначе переносится с прошлого патча с предупреждением. d2vpkr давал 7.41f = копия 7.41e (Earth Spirit/KotL/Warlock BAS). Тест: `tests/test_latest_patch_data.py`.
- `RELEASE_HISTORY` сортируется по дате (новые сверху) при загрузке `patch/meta.py` — порядок в файле больше не важен.
- Mana Items: история предметов с интеллектом считается по константе ТОГО патча (`_int_const_at`: 11 маны/Int в 7.36–7.38).
- Остаток аудита (не сделано): d2vpkr-история нейтралов/способностей может отставать на патч; 4 разных «последних версии»; движковые константы продублированы (heroes_stats / entity_changes / scripts.js); `_bucket` в terrain сравнивает только мажор.
