...
| `heroes_stats.html` | **Hero Stats** — one row per hero (127), every base stat. **Three View modes** (`#hs-view-mode` dropdown → `hs-mode-base/starting/expanded` on the table): **Base** = raw KV values; **Starting** (default) = level-1 values WITH attribute bonuses (HP=base+22Str, MP=base+12Int, HP/MP regen=base+0.1Str/0.05Int, Armor=base+Agi/6, MagRes=base+0.1Int, Damage=base+primary-attr (Universal = floor(mult × Σattrs), mult patch-gated 0.6/0.7/0.45), ASPD=base+Agi); **Expanded** = Starting + STR/AGI/INT 30, total attrs L1/L30, total gains, Min/Max dmg, projectile/turn/collision/bound. Cells whose value differs between Base & Starting carry `data-base-sort/-html/-hist`; the View IIFE (`scripts.js` "HERO STATS") swaps them. **Per-hero special cases:** Huskar MP/regen forced to 0; Ogre Magi mana/regen scale with Str. **Innate attribute-conversion modifiers** (`_innate_bonus`, patch-gated): Morphling Ebb&Flow Agi→Range/MoveSpeed (7.41+, 0.2→0.25 range @7.41d), Void Spirit Intrinsic Edge +33%/25% secondary bonuses (7.36/7.36b), Centaur Horsepower Str→MoveSpeed (7.36+). Add more heroes there. Every numeric cell carries full 7.08→today change history (`data-hist`+`data-net`); attribute cell logs primary-attr swaps. **Patch-note shift layer** (`_history_for` + `_load_patchnote_events`): the KV scrape is sometimes one sub-patch late, so KV-detected changes shift back to the patch that `patchnotes_english.txt` announces (Treant 7.34d "Base Damage decreased by 2" → KV shows it at 7.34e). Reuses mr-table front-end (`mr-table hs-table`). Frozen Hero column = CSS-sticky `.hs-name` + JS `.hs-sticky-frame` divider overlay (`scripts.js`). | `build_heroes_stats.py` |
