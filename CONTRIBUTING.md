...
- **Tag corrections.** The autodetector in `generate_patch_code_v2.py` mislabels ~10–20% of rows. Spot-check the rendered HTML against the original Valve patchnote and PR fixes.
...
- **New helpers / patterns.** If you find a recurring pattern that requires hand-editing every patch, extract a helper into the `patch/` package (e.g. `patch/elements.py`) and document it in `docs/`.

## Editing workflow

1. **Find the patch file.** Each patch lives in `content/p<version>.py` as a `def build():` whose body is a sequence of `W(hero_header(...))`, `W(ability(...))`, `W(li(...))`, etc. calls (built on the `patch/` helper API).
...
4. **Run the audits** (CI runs these too, but it's faster to catch them locally):
   ...
   import re, pathlib
   lines = []
   for p in sorted(pathlib.Path('content').glob('*.py')):
...
- `python builders/patch.py` must succeed before commit. CI fails otherwise.
