...
def render_html(rows: list[dict], cost_hist: dict[str, list] | None = None,
    ...
    thead = ''.join(
        ...
        f'<span class="th-label" data-i18n="mi.col.{k}">{label}</span>'
    ...
    return (
        ...
        # horizontal scroll.
        f'{subnav}'
        '<p class="mr-blurb inbox-bar">'
        '<span data-i18n-html="mi.blurb1">Every purchasable item that contributes mana regen, '
        'sorted by total <em>Mana regen</em> by default. Intelligence contributes</span> '
        f'{_int_const_chip("+12 max mana", _INT_MANA_HIST)} '
        '<span data-i18n="mi.blurb_and">and</span> '
        f'{_int_const_chip("+0.05 mana regen", _INT_REGEN_HIST)} '
        '<span data-i18n-html="mi.blurb2">per '
        ...
        'Click any column header to re-sort.</span></p>\n'
        ...
        '<span class="ua-upgrades-label" data-i18n="mi.hide_active">Hide Active</span>'
        ...
        'placeholder="Search items — blink, kaya, arcane…" data-i18n-placeholder="mi.search_ph">'
        '</span>'
        '</div></div>\n'
        f'{table}\n'
        '</div>\n'   # close .creeps-scroll
