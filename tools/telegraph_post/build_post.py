...
POST = [
    ...
    ("p", 'Sloppy — сайт о патчах Dota 2: каждое изменение размечено тегами, а в таблицах '
          ...
          'сменил облик, получил новые страницы, полностью размеченный патч 7.38, заметно '
          'ускорился и избавился от пачки ошибок в данных.'),
    ("links", [("Открыть сайт", SITE), ("Changelog сайта", SITE + "changelog.html")]),
    ("toc", None),

    ("h3", "Новый облик"),
    ("h4", "Шапка «Угольки»"),
    ("p", "Кнопки в шапке теперь просто текст. Текущая страница горит золотом, а из тонкой "
          "полоски под ней поднимаются искры — как у автокаста в игре. Раздел Materials "
          "подсвечен снизу тёплым отсветом, будто от углей."),
    ...
    ("p", "Меню, панели фильтров, переключатели и поля ввода перешли на один стиль: пиксельный "
          "шрифт, ступенчатые углы, «игровые» кнопки с тёмной кромкой. Нажатая кнопка с текстом "
          "заливается золотом, а кнопка-иконка — светится изнутри, чтобы иконку было видно."),
    ("img", ("2026-09-25_controls.webp", "Панель фильтров Hero Stats")),
    ("img", ("2026-09-25_menu.webp", "Выпадающее меню с пиксельными галочками")),
    ("img", ("2026-09-25_terrain_bar.webp", "Кнопки Terrain в том же стиле")),
    ("h4", "Тёплые цвета"),
    ("p", "Холодные стальные серые и синие ушли, их место заняли спокойные тёплые тона — "
          "фон, линии, шапки таблиц, календарь и его графики. Синий остался только там, где "
          "он что-то значит: QOL, магический урон, интеллект, Aghanim, мана."),
    ("img", ("2026-09-25_warm.webp", "Neutral Stats в новых цветах")),
    ("img", ("2026-09-25_warm_calendar.webp", "Календарь патчей")),
    ("h4", "Теги горят, как неон"),
    ("p", "Теги и группы на страницах патчей стоят в одну строку, без подписей «Tags» и "
          "«Group», их разделяет тонкая линия. Нажатый тег светится своим цветом — и буквы, и "
          "рамка. NEW и REWORK стали ярче, а DEL — винно-красным, чтобы не путать его с NERF."),
    ("img", ("2026-09-25_tags.webp", "NERF и Items нажаты")),
    ("h4", "Новые иконки"),
    ("p", "У каждой плитки главной страницы своя пиксельная иконка. Заново нарисованы "
          "ближний и дальний бой, таланты (светлое кольцо с веточкой, как в игре) и жук, "
          "которым в Changelog помечены исправления. А способности с автокастом в таблицах "
          "нейтралов показывают эффект из самой игры — огонь бежит по краю иконки."),
    ("img", ("2026-09-24_index.webp", "Главная страница")),
    ("img", ("2026-09-25_aoe_talent.webp", "AoE Increase: включённый талант и предметы")),
    ...
    ("img", ("2026-09-24_bounty.webp", "Золото с рун богатства — таблица по минутам")),
    ...
    ("p", "<b>Changelog</b> — всё новое на сайте по датам, со скриншотами в карусели. Мелкие "
          "правки собраны списком под днём, исправления помечены пиксельным жуком, а слева — "
          "оглавление только из крупных новинок."),
    ...
    ("ul", ["<b>Mana Items:</b> сколько даёт одно очко интеллекта (+12 маны, +0.05 регена) — "
            ...
            "характеристики. Добавлены Rubick (+1 урон за уровень), Tidehunter (+3 здоровья и "
    ...
    ("h3", "Быстрее"),
    ("p", "Hero Dynamics и Item Dynamics — это около 3000 ячеек. Раньше при прокрутке каждая "
          "ячейка, проезжавшая под курсором, запускала анимацию увеличения, и таблица "
          "перерисовывалась целиком. Теперь во время прокрутки наведение на паузе, а ячейка "
          "увеличивается мгновенно: медленных кадров при наведении стало в 7 раз меньше."),
    ...
    ("ul", ["Базовая скорость атаки в 7.41f застряла на значениях 7.41e — теперь верно: "
            ...
            "берутся прямо из файлов игры, и это проверяет тест.",
            ...
            "Врождённые способности 7.41f читаются правильно, и их список больше не расходится "
            "между страницами.",
            "Порядок патчей держится сам: новый патч можно дописать в любое место списка."]),
    ("hr", None),
