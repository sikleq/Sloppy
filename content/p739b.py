...
def build():
    ...
    W(ul_open())
    W(li("Pollinate health restoration loss increased from 20% to 30%", b(20, 30, l=True)))
    W(ul_close())
    W(item_header("Dezun Bloodrite"))
    W(ul_open())
    W(li("Abilities that are considered attacks are no longer impacted by Dezun Bloodrite (Including Outworld Destroyer's Arcane Orb, Viper's Poison Attack, etc.)", t("MISC")))
    ...
    W(ul_open())
    W(li("Bloodrage: Now has a 0.3s cast point when cast on units other than Bloodseeker himself", t("MISC")))
    ...
    W(ul_open())
    W(li("Eidolons created from multiplying will now automatically attack the same target as their parent", t("MISC")))
    W(ul_close())
    ...
    W(ul_open())
    W(li("Once again grants bonus stun and critical strike from marks generated by parrying an enemy Hero", t("MISC")))
    ...
    W(hero_header("Lich"))
    W(ul_open())
    W(li("Death Charge: Mana restored on Hero Death increased from 15% to 25%", b(15, 25)))
    W(ul_close())
    ...
    W(hero_header("Slark"))
    W(ul_open())
    W(li("Barracuda: Health Gained per second decreased from 10/70/100/130 to 5/70/100/130", b([10, 70, 100, 130], [5, 70, 100, 130])))
    W(ul_close())
