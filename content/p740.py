...
def build():
    ...
    W(ul_open())
    W(li("Now upgrades with each subsequent drop", t("REWORK"), extra=inline_note("Effect radius rescaled from 650 to 600/900/1200 — " + b(650, [600, 900, 1200]) + "<br>Hits to kill increased from 6 to 6/8/10 — " + b(6, [6, 8, 10]))))
    W(ul_close())
    ...
    W(ul_open())
    W(li("Affected creeps now gain damage buff from Rabble-Rouser for the duration in case they did not have it", t("NEW")))
    ...
    W(ul_open())
    W(li("Stun duration from falling off the cut tree decreased from 4s to 3s", b(4, 3, l=True)))
    W(ul_close())
    ...
    W(ul_close())
    W(ability("Backstab", slug="riki_innate_backstab", innate=True))
    W(ul_open())
    ...
    W(li("Mana per second rescaled from 8/12/16/20 to 9/12/15/18", b([8, 12, 16, 20], [9, 12, 15, 18], l=True)))
    W(ul_close())
